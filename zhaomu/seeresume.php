<?php
	...
	$filenames=$check->bianliDir($dirname);
?>
<!DOCTYPE html>
...
<div class="h2"><strong>简历列表</strong></div>
<div class="about_right_c">



<?php
	echo '<h2>共计'.count($filenames).'份简历</h2><br>';
		for($i=0; $i<count($filenames); $i++){
			$pathnames=explode("/", $filenames[$i]);
			$rnames=explode(".", $pathnames[count($pathnames)-1]);
			$rname=$rnames[count($rnames)-2];
			$namelenth=strlen($rname);
			for($l=0; $l<15-$namelenth; $l++){
				$rname.="&nbsp";
			}
			echo '<form action="interview.php" method="post">';
			echo '<p>编号'.($i+1).':'.$rname.'<input type="hidden" name="filename" value="'.$filenames[$i].'"><input type="image" align="middle" src="二七路   诚聘英才_files/chakan.png" alt="submit" /></p>';
			echo '</form>';	
			} 
